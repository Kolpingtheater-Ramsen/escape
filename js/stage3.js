...
function estimateMultiplePoses() {
  ...
  engine.detector.estimatePoses(canvas, estimationConfig).then((poses) => {
    ...
    const angleDiff = angles.map((e, i) =>
      e ? e - wantedAngles[i] : null
    )
    ...
    if (scaledSum < 5) {
      accBar.classList.add('green')
      score += 1 / frameRate
    } else {
      score = 0
      accBar.classList.add('yellow')
    }
